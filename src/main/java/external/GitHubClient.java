...
public class GitHubClient {
	...
	public List<Item> search(double lat, double lon, String keyword) {
		...
		try {
			keyword = URLEncoder.encode(keyword, "UTF-8"); // Dharma Alpha -> Dharma20%Alpha
		...
		try {
			...
			// input stream reader 可以一个一个读， 也可一段一段读但是不知道一段有多长
			// buffer reader 一段一段读，读完为止
	...
	private List<Item> getItemList(JSONArray array) {
		...
		for (int i = 0; i < keywords.size(); ++i) {
			...
			Set<String> set = new HashSet<String>(list); // deduplication
	...
	public static void main(String[] args) {
		...
		List<Item> list = client.search(37.38, -122.08, null);
		try {
			for (Item item : list) {
